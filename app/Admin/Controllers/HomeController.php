...
class HomeController extends Controller
{
    public function index()
    {
        return Admin::content(function (Content $content) {
            ...
            $content->header('首页');
            $content->description('首页详情');

            $content->row(function ($row) {
                $row->column(3, new InfoBox('用户', 'users', 'aqua', '/admin/auth/users', '1024'));
                $row->column(3, new InfoBox('购物车', 'shopping-cart', 'green', '/admin/orders', '150%'));
                $row->column(3, new InfoBox('文章', 'book', 'yellow', '/admin/articles', '2786'));
                $row->column(3, new InfoBox('文档', 'file', 'red', '/admin/files', '698726'));
            });
